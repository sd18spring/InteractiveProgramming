...
''' Takes a card picture and creates a flattened image of it.
...
# Press 'p' to take a picture
    ...
    blur = cv2.GaussianBlur(gray,(5,25),0)
    retval, threshold = cv2.threshold(blur,100,255,cv2.THRESH_BINARY)

    # Find contours and sort them by size
    dummy,cnts,hier = cv2.findContours(threshold,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
