...
# initialize pygame display, window, and sounds
pygame.init()
pygame.mixer.init()
sounda= pygame.mixer.Sound("slash.wav")
soundb= pygame.mixer.Sound("soundtrack.wav")
soundb.play()
sounda.set_volume(1)
soundb.set_volume(0.1)
...
def generateNewBall() :
    """ Initializes a fruit object with thresholded random properties.
    """
...
def genWaveBalls():
    """ Initializes a new wave of fruits to be thrown onscreen.
    """
    if len(nowFruits) == 0:
        for i in range(3):
            generateNewBall()
...
def isOnFruit(vector):
    """ Returns a truthy value containing the status of the cursor over a visible fruit.
    """
    x,y = vector
    for fruit in nowFruits:
        if math.pow(x-fruit.x,2) + math.pow(y-fruit.y,2) < math.pow(fruit.radius+30,2):
            return fruit
    return False
...
while True:
    ...
    # create a wave of targets
    genWaveBalls()

    # Render mouse cursor at linearly mapped location on screen
    pygame.draw.circle(display_a, (255,255,255), (int(cx)*2, int(cy)), 30, 0)
